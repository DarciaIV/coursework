...
currday=$(date +"%e")
currmon=$(date +"%m")
let currday=currday+1-1 # -2 if backup job starts between [18;23] due to UTC else [0;17] -1
if [ $currday -lt 10 ]; then
        currday=0$currday
fi
#echo -e $currday #DEV
cat /tmp/testvdp | grep "$curryea-$currmon-$currday" | grep Completed > /tmp/r.tmp
mv /tmp/r.tmp /tmp/testvdp # removes past entries
cat /tmp/testvdp >> /tmp/1
